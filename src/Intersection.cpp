...
void WaitingVehicles::permitEntryToFirstInQueue()
{
    ...
    auto prmFront = _promises.begin();
    auto vehicle_front = _vehicles.begin();
    prmFront->set_value();
    _promises.erase(prmFront); 
    _vehicles.erase(vehicle_front);
}
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    std::promise<void> prmsVehicleGrantedEntry;
    std::future<void> frtVehicleGrantedEntry = prmsVehicleGrantedEntry.get_future();
    _waitingVehicles.pushBack(vehicle, std::move(prmsVehicleGrantedEntry));
    
    frtVehicleGrantedEntry.wait();
